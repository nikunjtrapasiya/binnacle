...
field is a unit of work, and belongs in your tracker instead. Entries
leave the way they arrived, on evidence: a deploy entry when the stage
reports the commit, a decision when its ticket and PR are both closed.
...
|---|---|---|
| [logbook](logbook/README.md) | Remembers what one working session owes the next: parked work, deferred follow-ups, unverified claims. Briefs you when a session starts, asks before you close something out with work still open against it, and retires decisions once their ticket and PR are closed. | 0.4.2 |
