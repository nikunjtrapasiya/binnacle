...
class TestCheck(LedgerCase):
    ...
    def test_check_takes_both_keys_and_unions(self):
        """The second PR on a ticket has no entry naming its number."""
        self.add("t1", "unverified", ticket="ABC-1270", verify="v")
        self.add("p1", "parked", pr=999, resume="r")
        clear = self.run_cli("check", "--pr", "998", "--json")
        self.assertEqual(clear.returncode, 0)
        out = self.run_cli("check", "--pr", "999", "--ticket", "abc-1270",
                           "--json")
        self.assertEqual(out.returncode, 1)
        payload = json.loads(out.stdout)
        self.assertEqual({b["id"] for b in payload["blockers"]},
                         {"t1", "p1"})
        self.assertEqual(payload["key"], "PR #999 on ABC-1270")

    def test_check_refuses_no_key(self):
        out = self.run_cli("check")
        self.assertEqual(out.returncode, 2)
        self.assertIn("--ticket, --pr, or both", out.stderr)
...
RETIRE_TEST_CONFIG = {
    "version": 1,
    "retire": {"ticket": "true", "pr": "true"},
}


class TestRetire(LedgerCase):
    def setUp(self):
        super().setUp()
        path = os.path.join(self.tmp.name, "logbook.json")
        with open(path, "w") as fh:
            json.dump(RETIRE_TEST_CONFIG, fh)
        os.environ["LOGBOOK_CONFIG"] = path
        self.led = load_ledger()
        self.led.default_config_path = lambda: os.path.realpath(path)

    def add(self, eid, kind="decision", **kw):
        row = {"op": "add", "id": eid, "ts": days_ago(1), "kind": kind,
               "text": "t", "why": "because", "ticket": "ABC-1"}
        row.update(kw)
        self.led.append(row)

    def run_retire(self, verdicts, dry_run=False, ticket=None):
        buf = io.StringIO()
        with contextlib.redirect_stdout(buf):
            rc = self.led.cmd_retire(
                Args(ticket=ticket, dry_run=dry_run),
                closed=lambda entry, key: verdicts.get(key))
        self.assertEqual(rc, 0)
        return buf.getvalue()

    def status(self, eid):
        return self.led.replay(self.led.read_events())[eid]["status"]

    def test_closed_ticket_drops_a_decision(self):
        self.add("x1")
        out = self.run_retire({"ticket": True})
        self.assertEqual(self.status("x1"), "dropped")
        self.assertIn("dropping", out)
        entries = self.led.replay(self.led.read_events())
        self.assertIn("ticket ABC-1 closed", entries["x1"]["note"])

    def test_open_ticket_keeps(self):
        self.add("x1")
        out = self.run_retire({"ticket": False})
        self.assertEqual(self.status("x1"), "open")
        self.assertIn("still open", out)

    def test_unknown_holds(self):
        self.add("x1")
        out = self.run_retire({"ticket": None})
        self.assertEqual(self.status("x1"), "open")
        self.assertIn("could not determine", out)

    def test_both_keys_must_close(self):
        self.add("x1", pr=7)
        self.run_retire({"ticket": True, "pr": False})
        self.assertEqual(self.status("x1"), "open")
        self.run_retire({"ticket": True, "pr": True})
        self.assertEqual(self.status("x1"), "dropped")

    def test_pr_only_entry_needs_only_the_pr(self):
        self.add("x1", ticket=None, pr=7)
        self.run_retire({"pr": True, "ticket": False})
        self.assertEqual(self.status("x1"), "dropped")

    def test_keyless_entry_holds(self):
        self.add("x1", ticket=None)
        out = self.run_retire({"ticket": True, "pr": True})
        self.assertEqual(self.status("x1"), "open")
        self.assertIn("no ticket or pr", out)

    def test_blocking_kinds_are_never_touched(self):
        """Config cannot opt a blocking kind in: the check is a proof
        of closure, not of the obligation being met."""
        self.add("u1", kind="unverified", verify="v")
        self.add("f1", kind="followup")
        self.led.RETIRE["kinds"] = ["decision", "unverified", "followup"]
        self.run_retire({"ticket": True})
        self.assertEqual(self.status("u1"), "open")
        self.assertEqual(self.status("f1"), "open")

    def test_dry_run_writes_nothing(self):
        self.add("x1")
        before = len(self.led.read_events())
        out = self.run_retire({"ticket": True}, dry_run=True)
        self.assertEqual(len(self.led.read_events()), before)
        self.assertIn("would drop", out)

    def test_ticket_filter(self):
        self.add("x1", ticket="ABC-1")
        self.add("x2", ticket="ABC-2")
        self.run_retire({"ticket": True}, ticket="abc-2")
        self.assertEqual(self.status("x1"), "open")
        self.assertEqual(self.status("x2"), "dropped")

    def test_missing_resolver_holds_and_names_it(self):
        self.add("x1", pr=7)
        self.led.RETIRE["pr"] = ""
        out = self.run_retire({"ticket": True, "pr": True})
        self.assertEqual(self.status("x1"), "open")
        self.assertIn("add retire.pr", out)

    def test_disabled_resolvers_hold_everything(self):
        self.add("x1")
        self.led.default_config_path = lambda: os.path.join(
            self.tmp.name, "elsewhere", "logbook.json")
        out = self.run_retire({"ticket": True})
        self.assertEqual(self.status("x1"), "open")
        self.assertIn("resolvers disabled", out)

    def run_closed(self, closed, verdicts=None, dry_run=False):
        buf = io.StringIO()
        err = io.StringIO()
        with contextlib.redirect_stdout(buf), contextlib.redirect_stderr(err):
            rc = self.led.cmd_retire(
                Args(ticket=None, dry_run=dry_run, closed=closed),
                closed=lambda entry, key: (verdicts or {}).get(key))
        return rc, buf.getvalue(), err.getvalue()

    def test_closed_asserts_a_ticket_without_a_resolver(self):
        self.add("x1", ticket="ABC-1")
        self.add("x2", ticket="ABC-2")
        self.led.RETIRE["ticket"] = ""
        rc, out, _ = self.run_closed(["abc-1"])
        self.assertEqual(rc, 0)
        self.assertEqual(self.status("x1"), "dropped")
        self.assertEqual(self.status("x2"), "open")
        self.assertNotIn("x2", out)
        note = self.led.replay(self.led.read_events())["x1"]["note"]
        self.assertIn("ticket ABC-1 (asserted) closed", note)

    def test_closed_runs_no_resolver_and_needs_no_boundary(self):
        self.add("x1", ticket="ABC-1")
        self.led.default_config_path = lambda: os.path.join(
            self.tmp.name, "elsewhere", "logbook.json")
        called = []
        ...
        with contextlib.redirect_stdout(buf):
            self.led.cmd_retire(
                Args(ticket=None, dry_run=False, closed=["ABC-1"]),
                closed=lambda entry, key: called.append(key) or True)
        self.assertEqual(called, [])
        self.assertEqual(self.status("x1"), "dropped")

    def test_closed_ticket_still_asks_the_pr(self):
        self.add("x1", ticket="ABC-1", pr=7)
        rc, out, _ = self.run_closed(["ABC-1"], {"pr": False})
        self.assertEqual(self.status("x1"), "open")
        self.assertIn("pr still open", out)
        self.run_closed(["ABC-1", "7"])
        self.assertEqual(self.status("x1"), "dropped")

    def test_closed_pr_number_matches_the_pr_key(self):
        self.add("x1", ticket=None, pr=42)
        self.add("x2", ticket=None, pr=43)
        self.run_closed(["42"])
        self.assertEqual(self.status("x1"), "dropped")
        self.assertEqual(self.status("x2"), "open")

    def test_closed_refuses_a_token_that_is_neither(self):
        self.add("x1", ticket="ABC-1")
        rc, _, err = self.run_closed(["ABC-1,APG_12"])
        self.assertEqual(rc, 2)
        self.assertIn("APG_12", err)
        self.assertEqual(self.status("x1"), "open")

    def test_closed_never_touches_a_blocking_kind(self):
        self.add("u1", kind="unverified", verify="v", ticket="ABC-1")
        self.run_closed(["ABC-1"])
        self.assertEqual(self.status("u1"), "open")
...
class ConfigCase(ConfigBase):
    ...
    def test_doctor_refuses_a_blocking_kind_in_retire(self):
        led = self.trusted({"version": 1, "retire": {
            "ticket": "true", "kinds": ["decision", "unverified", "nope"]}})
        findings = led.config_findings()
        self.assertTrue(any("'unverified', which blocks" in f
                            for f in findings))
        self.assertTrue(any("'nope', which is not a declared" in f
                            for f in findings))
        self.assertFalse(any("nothing retirable" in f for f in findings))

    def test_doctor_flags_retire_wired_with_nothing_retirable(self):
        led = self.trusted({"version": 1, "retire": {
            "ticket": "true", "kinds": ["unverified"]}})
        self.assertTrue(any("nothing retirable" in f
                            for f in led.config_findings()))

    def test_doctor_flags_retire_resolvers_that_will_not_run(self):
        led = self.write({"version": 1, "retire": {"ticket": "true"}})
        self.assertTrue(any("retire.ticket or retire.pr is configured but"
                            in f for f in led.config_findings()))
        quiet = self.write({"version": 1})
        self.assertFalse(any("retire" in f
                             for f in quiet.config_findings()))
...
class ResolverCase(ConfigBase):
    ...
    def test_key_closed_three_states(self):
        closed = self.script("closed.sh", "exit 0")
        still = self.script("open.sh", "exit 1")
        unknown = self.script("huh.sh", "exit 2")
        led = self.trusted({"version": 1, "home": self.tmp.name,
                            "retire": {"ticket": closed, "pr": still}})
        row = self.entry(kind="decision", ticket="ABC-1", pr=7)
        self.assertIs(led.key_closed(row, "ticket"), True)
        self.assertIs(led.key_closed(row, "pr"), False)
        led.RETIRE["pr"] = unknown
        self.assertIsNone(led.key_closed(row, "pr"))
        led.RETIRE["pr"] = ""
        self.assertIsNone(led.key_closed(row, "pr"))

    def test_retire_resolver_sees_pr_and_repo_path(self):
        marker = os.path.join(self.tmp.name, "seen.txt")
        path = self.script(
            "env.sh", 'echo "$LOGBOOK_PR $LOGBOOK_REPO_PATH" > %s' % marker)
        led = self.trusted({"version": 1, "home": self.tmp.name,
                            "retire": {"ticket": path, "pr": path}})
        row = self.entry(pr=42, repo_path="/tmp/checkout")
        led.key_closed(row, "pr")
        with open(marker) as fh:
            self.assertEqual(fh.read().strip(), "42 /tmp/checkout")

    def test_retire_timeout_is_its_own(self):
        path = self.script("slow.sh", "sleep 5")
        led = self.trusted({"version": 1, "home": self.tmp.name,
                            "retire": {"timeout_seconds": 1,
                                       "ticket": path}})
        started = time.time()
        self.assertIsNone(led.key_closed(self.entry(ticket="ABC-1"),
                                         "ticket"))
...
class HookCase(ConfigBase):
    ...
    def ticket_branch(self, name):
        repo = os.path.join(self.tmp.name, "repo")
        os.makedirs(repo)
        env = dict(os.environ, GIT_AUTHOR_NAME="t", GIT_AUTHOR_EMAIL="t@x",
                   GIT_COMMITTER_NAME="t", GIT_COMMITTER_EMAIL="t@x")
        for argv in (["git", "init", "-q", "-b", name],
                     ["git", "commit", "-q", "--allow-empty", "-m", "x"]):
            subprocess.run(argv, cwd=repo, env=env, check=True,
                           capture_output=True)
        return repo

    def test_gate_asks_on_numbered_merge_via_branch_ticket(self):
        """Second PR on a ticket: no entry names its number yet.

        The check was keyed by the number alone whenever one was given,
        and fell back to the branch ticket only without one, so every
        ticket-keyed blocker was missed and the merge went through.
        """
        self.open_entry(kind="unverified", verify="v", ticket="ABC-1")
        repo = self.ticket_branch("abc-1-second-pr")
        out = self.run_gate({"tool_name": "Bash", "cwd": repo,
                             "tool_input": {"command": "gh pr merge 999"}},
                            cwd=repo)
        self.assertEqual(out.returncode, 0)
        self.assertIn("ask", out.stdout)
        self.assertIn("ABC-1", out.stdout)

    def test_gate_numbered_merge_off_ticket_branch_still_keyed_by_pr(self):
        self.open_entry(pr=7)
        repo = self.ticket_branch("main")
        out = self.run_gate({"tool_name": "Bash", "cwd": repo,
                             "tool_input": {"command": "gh pr merge 7"}},
                            cwd=repo)
        self.assertIn("ask", out.stdout)
