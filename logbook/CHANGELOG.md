...
Reported by a colleague after a merge went through with an open entry
against its ticket.

### Fixed

- The gate keyed a `gh pr merge <N>` check by N alone, and fell back to
  the branch ticket only when no number was given. The second PR on a
  ticket is merged before any entry names its number - entries are
  written while the work is happening, the PR comes after - so a check
  keyed by the number found nothing and every ticket-keyed blocker was
  missed. The gate now sends both the number and the branch ticket, and
  `check` accepts `--ticket` and `--pr` together and unions the
  blockers; the key it prints reads `PR #N on ABC-123`. `check` with
  neither key is refused (exit 2) rather than an argparse error.

## [0.4.1] - 2026-09-23

### Added

- `retire --closed K,K,N` takes ticket keys and PR numbers the caller
  has already confirmed closed, and retires the entries carrying them
  without running a resolver. The case: the session's own tracker tool
  (an MCP tool, callable by the model and by nothing else) can answer
  "is this Done" in one query, but a shell script cannot reach it. So
  the model asks, then hands the answer in. An asserted key executes
  nothing, so it needs no config boundary; any other key on the entry
  is still asked of its resolver. The drop note marks the source,
  `ticket ABC-123 (asserted) closed`, so the two kinds of evidence stay
  distinguishable in the history. A token that is neither a key nor a
  number is refused (exit 2) rather than skipped, because a typo that
  silently held an entry would read as "still open".

## [0.4.0] - 2026-09-23

A ledger that only grows stops being read. After a month of use the
brief carried 140 open `decision` entries against 50 obligations, most
on tickets long since Done, and the obligations were being skimmed past
with them.

### Added

- `logbook retire [--ticket K] [--dry-run]` drops non-blocking entries
  whose ticket and PR are both closed, on evidence from the tracker and
  the forge rather than on age. A new `retire` config section names two
  resolvers, `retire.ticket` and `retire.pr`, each with the tri-state
  contract `exists` already uses: exit 0 closed, exit 1 still open,
  anything else holds. Every key an entry carries must answer closed;
  an entry with neither key holds and says so. `retire.kinds` (default
  `["decision"]`) can only name kinds declared non-blocking, because a
  closed ticket does not prove an obligation was met; `doctor` flags a
  blocking or undeclared kind there, and a `retire` resolver that will
  not run because the config is not at the trusted path.
- Resolvers now also receive `LOGBOOK_PR` and `LOGBOOK_REPO_PATH`, so a
  PR check can run in the checkout the entry came from.
- `examples/ticket-closed-jira.sh` and `examples/pr-closed-gh.sh`, both
  multi-outcome scripts for the same reason the deploy examples are:
  `gh` and `curl` exit non-zero for a missing token and for a missing
  record alike, so "closed" is read off the response, never the exit
  code.
