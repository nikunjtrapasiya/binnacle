...
logbook check (--ticket K | --pr N) [--json]
...
- **`check`** is the close-out gate: exit 1 when a blocking entry is open
  against the key, exit 0 otherwise. Exit 0 still prints any `decision`
...
are run with a shell (`subprocess.Popen(..., shell=True)`, so a timeout can
...
`deploy-sync` holds every entry when resolvers are disabled and prints why.
`doctor` always reports the effective config path, and reports that
resolvers will not run whenever `deploy.stages` is configured and the config
in force is not the trusted default one.
