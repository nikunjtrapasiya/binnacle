...
def main():
    ...
    if name == "Bash":
        ...
        if matched:
            # both keys, not one or the other: the second PR on a ticket
            # is merged before any entry names its number, so a check
            # keyed by the number alone found nothing and let it through
    ...
    if tool.CONFIG_SOURCE["fatal"]:
        return emit(gate, "logbook cannot check %s: config refused: %s"
                          % (" ".join(str(k) for k in key),
                             tool.CONFIG_SOURCE["fatal"]))

    out = subprocess.run([sys.executable, LEDGER, "check"]
                         + [str(k) for k in key] + ["--json"],
